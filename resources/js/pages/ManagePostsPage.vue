        ...
        <section class="rounded-3xl border border-zinc-200 bg-white p-8 shadow-xl">
          ...
          <div class="mt-8 overflow-hidden rounded-2xl border border-zinc-200">
            ...
            <div v-if="isLoading" class="space-y-3 bg-white p-5">
              <div v-for="item in 4" :key="item" class="h-20 animate-pulse rounded-2xl bg-zinc-100"></div>
            </div>

            <div v-else-if="posts.length" class="divide-y divide-zinc-200 bg-white">
              <div v-for="post in posts" :key="post.id" class="grid grid-cols-12 gap-4 px-5 py-5">
                <div class="col-span-12 md:col-span-5">
                  <div class="flex items-center gap-4">
                    <div class="h-16 w-16 overflow-hidden rounded-2xl bg-zinc-100">
                      ...
                      />
                      <div v-else class="flex h-full items-center justify-center bg-zinc-200 text-[10px] font-medium text-zinc-500">
                        Yok
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <p class="mt-1 text-sm text-zinc-500">{{ post.slug }}</p>
                    </div>
                  </div>
                </div>
            ...
            <div v-else class="bg-white px-6 py-14 text-center">
              <p class="text-lg font-semibold text-zinc-900">Henüz listelenecek yazı yok</p>
              <p class="mt-2 text-sm text-zinc-500">Yeni bir yazı eklediğinde burada gözükecek.</p>
            </div>
          </div>
        </section>
      </div>
